...
from smartodds.evaluation.evaluation import EvaluationMetrics, metric_to_function, rmse
from smartodds.model.utils import create_match_array
...
class BaseModelClass(ABC):
    ...
    def _prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, dict[str, int]]:
        ...
        team_to_idx = {team: idx for idx, team in enumerate(unique_fitset_teams)}
        ...
        testset = testset[
            testset["Home"].isin(unique_fitset_teams)
            & testset["Away"].isin(unique_fitset_teams)
        ]
    ...
    def fit(self):
        ...
        bounds = self.bounds()
        result = minimize(
            self.log_likelihood, initial_params, method="L-BFGS-B", bounds=bounds
        )
        self._result = result
    ...
    def evaluate(self) -> dict[str, floating[Any]]:
        ...
        metrics["total_goals_rmse"] = rmse(
            total_goals, self.test["total_expected_goals"]
        )
    ...
    def _calculate_expected_points(
        win_probability: float, draw_probability: float
    ) -> float:
    ...
    def create_expected_points_table(self):
        ...
        expected_points_table = (
            self._test.groupby("Home")["home_expected_points"].sum()
            + self._test.groupby("Away")["away_expected_points"].sum()
        )
        # order and add position
        expected_points_table = expected_points_table.sort_values(ascending=False)
        expected_points_table = expected_points_table.reset_index()
        return expected_points_table

    def _prepare_comparison_data(self):
        """Prepare the comparison data for the model."""
        ...
        comparison_data["home_idx"] = comparison_data["Home"].map(self.team_to_idx)
        comparison_data["away_idx"] = comparison_data["Away"].map(self.team_to_idx)

        comparison_data["total_expected_goals"] = (
            comparison_data["expected_team1_goals"]
            + comparison_data["expected_team2_goals"]
        )
        ...
        comparison_data = comparison_data.merge(
            ...
            on=["home_idx", "away_idx", "Date"],
            how="inner",
        )
        return comparison_data

    def evaluate_comparison_data(self):
        """Evaluate the comparison data using the test set."""
        ...
        metrics["total_goals_rmse"] = rmse(
            total_goals, self.compare_data["total_expected_goals"]
        )
    ...
    def create_league_table(
        self, season_results: np.ndarray, num_teams: int
        ...
        Notes:
            The league tables are sorted by points, goal difference, goals scored, and team
            name for alphabetical order.
        """
        points_eastern = np.zeros(num_teams, dtype=int)
        points_western = np.zeros(num_teams, dtype=int)
        goal_diff_eastern = np.zeros(num_teams, dtype=int)
        goal_diff_western = np.zeros(num_teams, dtype=int)
        goals_scored_eastern = np.zeros(num_teams, dtype=int)
        goals_scored_western = np.zeros(num_teams, dtype=int)
        ...
        for match in season_results:
            ...
            if home_conference == "Eastern":
                goals_scored_eastern[home_idx] += home_goals
                goal_diff_eastern[home_idx] += home_goals - away_goals
            elif home_conference == "Western":
                goals_scored_western[home_idx] += home_goals
                goal_diff_western[home_idx] += home_goals - away_goals

            if away_conference == "Eastern":
                goals_scored_eastern[away_idx] += away_goals
                goal_diff_eastern[away_idx] += away_goals - home_goals
            elif away_conference == "Western":
                goals_scored_western[away_idx] += away_goals
                goal_diff_western[away_idx] += away_goals - home_goals
        ...
        team_names = np.array(list(self.team_to_idx.keys()))

        table_eastern = np.column_stack(
            (
                np.arange(num_teams),
                points_eastern,
                goal_diff_eastern,
                goals_scored_eastern,
            )
        )
        table_western = np.column_stack(
            (
                np.arange(num_teams),
                points_western,
                goal_diff_western,
                goals_scored_western,
            )
        )

        sorted_indices_eastern = np.lexsort(
            (
                team_names[table_eastern[:, 0]],
                # Team name for alphabetical order
                table_eastern[:, 3],  # Goals scored
                table_eastern[:, 2],  # Goal difference
                -table_eastern[:, 1],  # Points (negative for descending order)
            )
        )

        sorted_indices_western = np.lexsort(
            (
                team_names[table_western[:, 0]],
                # Team name for alphabetical order
                table_western[:, 3],  # Goals scored
                table_western[:, 2],  # Goal difference
                -table_western[:, 1],  # Points (negative for descending order)
            )
        )

        table_eastern = table_eastern[sorted_indices_eastern]
        table_western = table_western[sorted_indices_western]

        return table_eastern, table_western

    def run_simulations(self, num_simulations: int) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        (
            ...
            team_1_conferences,
            team_2_conferences,
        ) = create_match_array(self.test)
        num_teams = len(self.team_to_idx)
        final_positions_eastern = np.zeros((num_simulations, num_teams), dtype=int)
        final_positions_western = np.zeros((num_simulations, num_teams), dtype=int)
    ...
    def get_team__name_position_probability(
        self, team: str, position_range: Tuple[int, int]
    ) -> Tuple[float, float]:
        """
        ...
        eastern_prob = np.sum(
            self.position_probabilities_eastern[team_id, start_pos - 1 : end_pos]
        )
        western_prob = np.sum(
            self.position_probabilities_western[team_id, start_pos - 1 : end_pos]
        )
